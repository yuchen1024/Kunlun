...
void test_psu(std::string party, size_t LEN) 
{
    ...
    if(party == "sender"){
        ...
        NetIO server("server", "192.168.0.12", 8080); 
        PSU::PipelineSender(server, pp, vec_X, LEN);
    ...
    if(party == "receiver")
    {
        ...
        PSU::PipelineReceiver(client, pp, vec_Y, LEN, unionXY_prime); 
        auto end_time = std::chrono::steady_clock::now(); 
        auto running_time = end_time - start_time;
        std::cout << "PSU takes time= " 
        << std::chrono::duration <double, std::milli> (running_time).count() << " ms" << std::endl;
...
int main()
{
    ...
    // auto end_time = std::chrono::steady_clock::now(); 
    // auto running_time = end_time - start_time;
    // std::cout << "PSU takes time (2^10 scale) = " 
    // << std::chrono::duration <double, std::milli> (running_time).count() << " ms" << std::endl;
