...
int main()
{
    ...
    PrintSplitLine('-'); 
    std::cout << "rrPKE-based mqRPMT test begins >>>" << std::endl; 
    PrintSplitLine('-'); 
    ...
    std::string pp_filename = "rrPKEmqRPMT.pp"; 
    rrPKEmqRPMT::PP pp; 
    if(!FileExist(pp_filename)){
        pp = rrPKEmqRPMT::Setup(statistical_parameter, LOG_SERVER_LEN, LOG_CLIENT_LEN); 
        rrPKEmqRPMT::SavePP(pp, pp_filename); 
    }
    else{
        rrPKEmqRPMT::FetchPP(pp, pp_filename); 
    }

    std::cout << "size of Server's set = " << pp.SERVER_LEN << std::endl; 
    std::cout << "size of Client's set = " << pp.CLIENT_LEN << std::endl; 

    std::string testcase_filename = "rrPKEmqRPMT.testcase"; 
    RPMTTestcase testcase; 
    ...
    if(party == "server"){
        ...
        vec_indication_bit_real = rrPKEmqRPMT::Server(server, pp, testcase.vec_Y); 
        ...
        std::cout << "rrPKE-based mqRPMT test succeeds with probability " << (1 - error_probability) << std::endl; 
    }

    if(party == "client")
    {
        NetIO client("client", "127.0.0.1", 8080); 
        rrPKEmqRPMT::Client(client, pp, testcase.vec_X);
    } 

    PrintSplitLine('-'); 
    std::cout << "rrPKE-based mqRPMT test ends >>>" << std::endl; 
    PrintSplitLine('-'); 
...
#else
    std::cout << "We haven't provided _X25519 version for this!" << std::endl; 
#endif    
}
