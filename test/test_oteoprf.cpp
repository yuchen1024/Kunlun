...
struct OTEOPRFTestCase{
    ...
    size_t INPUT_LEN; // size of set 
};

OTEOPRFTestCase GenTestCase(size_t LOG_INPUT_LEN)
{
    OTEOPRFTestCase testcase;
    testcase.INPUT_LEN = 1 << LOG_INPUT_LEN; 

    PRG::Seed seed = PRG::SetSeed(fixed_seed, 0); // initialize PRG
    testcase.vec_X = PRG::GenRandomBlocks(seed, testcase.INPUT_LEN);
...
void FetchTestCase(OTEOPRFTestCase &testcase, std::string testcase_filename)
{
    ...
    fin >> testcase.INPUT_LEN; 

    testcase.vec_X.resize(testcase.INPUT_LEN); 
...
int main()
{
    ...
    std::cout << "number of elements = " << (1 << LOG_INPUT_LEN) << std::endl; 

    std::string testcase_filename = "OTEOPRF.testcase"; 
    
    OTEOPRFTestCase testcase; 
    if(!FileExist(testcase_filename)){
        testcase = GenTestCase(LOG_INPUT_LEN); 
	...
	if (party == "server")
	{
        ...
        std::vector<std::vector<uint8_t>> vec_Fk_X = OTEOPRF::Evaluate(pp, oprf_key, testcase.vec_X, pp.INPUT_LEN);        
    }
    
    if (party == "client")
	{
        NetIO client_io("client", "127.0.0.1", 8080);
        std::vector<std::vector<uint8_t>> vec_Fk_X = OTEOPRF::Client(client_io, pp, testcase.vec_X, pp.INPUT_LEN);
	}
