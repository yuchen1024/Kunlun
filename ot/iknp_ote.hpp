...
void Send(NetIO &io, PP &pp, std::vector<block> &vec_m0, std::vector<block> &vec_m1, size_t EXTEND_LEN) 
{
    ...
    */
    ...
    std::cout << "Phase 1: sender has obliviuosly get "<< BASE_LEN << " number of keys" << std::endl; 
    ...
    */
    ...
    std::vector<block> vec_m(ROW_NUM/128);   // the plaintext
    ...
    for(auto j = 0; j < BASE_LEN; j++){
        ...
        if(vec_selection_bit[j] == 0){
            vec_m = Block::XOR(vec_inner_C0, vec_pad);
        }
        else{
            vec_m= Block::XOR(vec_inner_C1, vec_pad);   
        } 
        Block::ToDenseBits(vec_m.data(), ROW_NUM/128, Q.data()+j*ROW_NUM/8, ROW_NUM); 
    }   

    std::cout << "Phase 1: sender gets "<< BASE_LEN << " pair of ciphertexts" << std::endl; 

    // transpose Q
    std::vector<uint8_t> Q_tanspose(ROW_NUM/8 * COLUMN_NUM); 
    BitMatrixTranspose(Q.data(), ROW_NUM, COLUMN_NUM, Q_tanspose.data());  

    std::cout << "Phase 2: sender transposes matrix Q" << std::endl; 
    ...
    std::cout << "Phase 2: sender sends "<< ROW_NUM << " pair of ciphertexts" << std::endl;  
}
...
void Receive(NetIO &io, PP &pp, std::vector<block> &vec_result, const std::vector<uint8_t> &vec_selection_bit, size_t EXTEND_LEN)
{
    ...
    std::cout << "Phase 1: receiver has obliviuosly transmit "<< BASE_LEN << " number of keys" << std::endl; 
    ...
    std::cout << "Phase 1: receiver obliviuosly transmits "<< COLUMN_NUM << " pair of ciphertexts" << std::endl; 
    
    std::vector<uint8_t> T_transpose(ROW_NUM/8 * COLUMN_NUM); 
    BitMatrixTranspose(T.data(), ROW_NUM, COLUMN_NUM, T_transpose.data());

    std::cout << "receiver transposes matrix T" << std::endl; 
    ...
    std::cout << "Phase 2: receiver get "<< ROW_NUM << " pair of ciphertexts" << std::endl; 
