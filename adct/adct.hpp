...
Account CreateAccount(PP &pp, std::string identity, BigInt &init_balance, BigInt &sn)
{
    Account newAcct;
...
std::string ExtractSignMessageFromCTx(ToOneCTx &newCTx)
{
    std::string str;
...
ToOneCTx CreateCTx(PP &pp, Account &Acct_sender, BigInt &v, ECPoint &pkr)
{
    ...
    transcript_str += ExtractSignMessageFromCTx(newCTx);
...
std::string ExtractSignMessageFromCTx(ToManyCTx &newCTx)
{
    std::string str;
