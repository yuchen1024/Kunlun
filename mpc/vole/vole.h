...
namespace VOLE {
	...
	std::vector<block> VOLE_A(NetIO &A_io, uint64_t N_item, std::vector<block>& vec_C, uint64_t t = 32);
	void VOLE_B(NetIO &B_io, uint64_t N_item, std::vector<block>& vec_B, block delta, uint64_t t =32);
	...
	void tmpVOLE_B(NetIO &server_io, uint64_t N_item, uint64_t t, std::vector<block> vec_v, std::vector<block>& vec_leaf) {
		...
		ExConvCode ECEncode;
		ECEncode.config(ec_seed,2,21,10);
