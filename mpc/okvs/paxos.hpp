...
public:
   ...
   */
   ...
   // The backfill algorithm for a OKVS whose dense_type is gf_128 and value_type is MyStruct(block[]).
   void backfill_MyStruct128(value_type *values, value_type *output, PRG::Seed *prng);
};
...
template <typename idx_type, DenseType dense_type, typename value_type>
void OKVS<idx_type, dense_type, value_type>::backfill_MyStruct128(value_type *values, value_type *output, PRG::Seed *prng)
{
...
void OKVS<idx_type, dense_type, value_type>::encode(value_type *values,value_type *output, PRG::Seed *prng)
{
   ...
   else if (dense_type == gf_128 && std::is_same<value_type, MyStruct>::value)
      backfill_MyStruct128(values, output, prng);
...
PRG::Seed seed = PRG::SetSeed();
auto k = PRG::GenRandomBlocks(seed, 20);
std::vector<MyStruct> v(2);



//memcpy(v.data(), &k[0], sizeof(MyStruct) * 2);

//std::vector<uint8_t> buffer2(128);

//memcpy(buffer2.data(), &v[0], sizeof(MyStruct));
//MyStruct v0 = ((MyStruct*)(&buffer2[0]))[0];
...
for(auto i = 0; i< 16;++i){
//Block::PrintBlock(k[i]);
//std::cout << " " << std::endl;
}

//v[0] = ((MyStruct*)(&k[0]))[0];
//Block::PrintBlock(k[15]);
//Block::PrintBlock(v[1].var[7]);
//block* var = gf128_mul(v[0],k[16],8);
//v[1] = ((MyStruct*)(&var[0]))[0];
//v[1].printMyStruct();

//std::cout << MyStruct.len() << std::endl;

//MyStruct v = ((MyStruct*)(&k[0]))[0];
//MyStruct var;
...
//var.printMyStruct();
//block a[8];
//MyStruct v1 = ((MyStruct*)(&a))[0];   
//MyStruct v2 = ((MyStruct*)(&a[0]))[0];
...
/**
   ...
   std::vector<block> random_values = PRG::GenRandomBlocks(seed, 8*n);
   
   std::vector<MyStruct> v(n);
   for (auto i = 0; i < n; i++)
   {
      for(auto j = 0; j< 8; ++j){
      	v[i].var[j] = random_values[8*i+j];
      }
   }
   ...
   a.decode(k.data(), n, output.data(), temp_blocks.data());
   ...
   for(auto i = 0; i < 25; i++)  
   {
   	Block::PrintBlock(output[i].var[7]);
   	Block::PrintBlock(v[i].var[7]);
   	Block::PrintBlock(temp_blocks[i].var[7]);
   	std::cout << "" << std::endl;
