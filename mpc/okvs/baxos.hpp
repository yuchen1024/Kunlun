...
void test_baxos_MyStruct()
{
    ...
    std::vector<MyStruct> out(t.bin_num * t.total_size);
    std::vector<MyStruct> v(n);
    std::vector<MyStruct> v2(n);
    
    PRG::Seed seed = PRG::SetSeed(fixed_seed, 0);
    std::vector<block> k = PRG::GenRandomBlocks(seed, n);
    std::vector<block> v_pre = PRG::GenRandomBlocks(seed, n*(sizeof(MyStruct)/sizeof(block)));
    memcpy(v.data(), &v_pre[0], sizeof(MyStruct) * n);
    
    auto start = std::chrono::steady_clock::now();
    Baxos<gf_128, MyStruct> baxos(n, bin_size, 3);
    ...
    Baxos<gf_128, MyStruct> baxos2(n, bin_size, 3);
