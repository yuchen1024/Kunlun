...
/** @file
*****************************************************************************
This is an implementation of multi-query RPMT based on Rerandomizable PKE, mainly using ElGamal and OKVS::Baxos.

Note that the (ElGamal::CT) can be tansfered to (unsigned char [130]), as the efficient implementation of OKVS relys on block operations, we define MyStruct(block[9]) to save the (ElGamal::CT).
...
MyStruct CTtoMyStruct(ElGamal::CT &ct){
    std::vector<unsigned char> ct_buffer(2*POINT_BYTE_LEN);   
    std::vector<unsigned char> refill_buffer(sizeof(MyStruct),0);    
    ct_buffer = ElGamal::CTtoByteArray(ct);
    memcpy(refill_buffer.data(), ct_buffer.data(), 2*POINT_BYTE_LEN);
	return ((MyStruct*)(&refill_buffer[0]))[0];
}

ElGamal::CT MyStructToCT(MyStruct &mystruct){
 	std::vector<unsigned char> buffer(POINT_BYTE_LEN*2);
	memcpy(buffer.data(), &mystruct ,POINT_BYTE_LEN*2);
...
std::vector<uint8_t> Server(NetIO &io, PP &pp, std::vector<block> &vec_Y){
    ...
    uint64_t VALUE_BYTE_LEN = sizeof(MyStruct);    
    
    // transfer CT[] to MyStruct[]
    std::vector<MyStruct> vec_value(pp.SERVER_LEN);
    #pragma omp parallel for num_threads(NUMBER_OF_THREADS)
    for(auto i = 0; i < pp.SERVER_LEN; i++){
    	vec_value[i] = CTtoMyStruct(ct[i]);
    }
            
    //encode(vec_y, vec_value) to get vec_out
    uint64_t bin_size = 1 << (pp.LOG_SERVER_LEN-7);
    Baxos<gf_128, MyStruct> baxos(pp.SERVER_LEN, bin_size, 3, pp.statistical_security_parameter);
    auto out_length = baxos.bin_num * baxos.total_size;//calculate the output length of OKVS::decode(vec_Y,vec_out)
    std::vector<MyStruct> vec_out(out_length);
    ...
    std::vector<MyStruct> vec_rerand(pp.CLIENT_LEN);
    ...
    for(auto i = 0; i < pp.CLIENT_LEN; i++){
    	ElGamal::CT ct = MyStructToCT(vec_rerand[i]);
...
{    
    ...
    std::vector<MyStruct> vec_decode(pp.CLIENT_LEN);
    uint8_t thread_num = NUMBER_OF_THREADS;
    baxos.decode(vec_X, vec_decode, vec_out, thread_num);
    
    // re-rand the cipher and transfer it to MyStruct
    std::vector<MyStruct> vec_rerand(pp.CLIENT_LEN);
    #pragma omp parallel for num_threads(NUMBER_OF_THREADS)
    for(auto i = 0; i < pp.CLIENT_LEN; ++i){
    	ElGamal::CT tmp_ct = MyStructToCT(vec_decode[i]);
    	ElGamal::CT new_ct = ElGamal::ReRand(pp_elgamal, pk, tmp_ct);
	vec_rerand[i] = CTtoMyStruct(new_ct);
    }
