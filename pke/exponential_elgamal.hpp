#ifndef EXPONENTIAL_ELGAMAL_HPP_
#define EXPONENTIAL_ELGAMAL_HPP_
...
std::ofstream &operator<<(std::ofstream &fout, const ExponentialElGamal::PP &pp)
{
...
std::ifstream &operator>>(std::ifstream &fin, ExponentialElGamal::PP &pp)
{
...
{ 
    ...
    #ifdef PRINT
        std::cout << "generate the public parameters for Exponential ElGamal >>>" << std::endl; 
        PrintPP(pp); 
    #endif
...
{ 
    ...
    #ifdef DEBUG
        std::cout << "Exponential ElGamal encryption finishes >>>"<< std::endl;
        PrintCT(ct); 
    #endif
...
{ 
    ...
    if(SUCCESS == false)
    {
        /* dec the cipher that encrypted value is negative,
        this approach is added for SDPT this special situation which encrypted value is negative sometimes,
...
{  
    ...
    #ifdef DEBUG
        std::cout << n <<"-recipient 1-message Exponential ElGamal encryption finishes >>>"<< std::endl;
        PrintCT(ct); 
    #endif
